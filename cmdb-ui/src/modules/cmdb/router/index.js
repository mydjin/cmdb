...
const genCmdbRoutes = async () => {
  const routes = {
    ...
    children: [
      ...
      {
        path: '/spic-cmdb/disabled1',
        name: 'cmdb_disabled1',
        meta: { title: 'cmdb.menu.resources', disabled: true },
      },
      {
        path: '/spic-cmdb/relationviews/:viewId?',
      ...
      },
      {
        ...
        children: [
          {
            path: '/spic-cmdb/tree_views/:typeId',
      ...
      {
        path: `/spic-cmdb/cidetail/:typeId/:ciId`,
        name: 'cmdb_ci_detail',
        hidden: true,
      ...
      {
        path: '/spic-cmdb/disabled2',
        name: 'cmdb_disabled2',
        meta: { title: 'cmdb.menu.config', disabled: true, },
      },
      {
        path: '/spic-cmdb/preference',
      ...
      },
      {
        ...
        children: [
          {
            path: '/spic-cmdb/customdashboard',
          ...
          {
            path: '/spic-cmdb/preferencerelation',
          ...
          {
            path: '/spic-cmdb/operationhistory',
  ...
  preference.group_types.forEach(group => {
    if (preference.group_types.length > 1) {
      routes.children[resourceViewsIndex].children.push({
        path: `/spic-cmdb/instances/types/group${group.id}`,
    ...
    group.ci_types.forEach(item => {
      routes.children[resourceViewsIndex].children.push({
        path: `/spic-cmdb/instances/types/${item.id}`,
  ...
  if (lastTypeId && preference.type_ids.some(item => item === Number(lastTypeId))) {
    routes.redirect = `/spic-cmdb/instances/types/${lastTypeId}`
    ...
    routes.redirect = '/spic-cmdb/dashboard'
  }
